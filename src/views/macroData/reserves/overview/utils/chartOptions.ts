...
export function useGoldAnalysisOption(trend: RaTrend | null): EChartsOption {
  ...
  return {
    ...
    grid: { left: 90, right: 140, top: 55, bottom: 30, containLabel: false },
    xAxis: categoryAxis(dates),
    yAxis: [
      { ...valueAxis('万盎司', 'left'), splitLine: { lineStyle: { type: 'dashed', color: '#f0f0f0' } } },
      { ...valueAxis('万盎司(增量)', 'right'), offset: 0, splitLine: { show: false } },
      { ...valueAxis('亿美元', 'right'), offset: 55, splitLine: { show: false } },
      { ...valueAxis('%', 'left'), offset: 55, splitLine: { show: false } },
    ],
    dataZoom: [{ type: 'inside', start: 0, end: 100 }],
    series: [
      {
        ...
        lineStyle: { width: 2.5, color: COLORS.blue },
        itemStyle: { color: COLORS.blue },
        areaStyle: { opacity: 0.08, color: COLORS.blue },
      ...
      },
      {
        ...
        type: 'line',
        yAxisIndex: 3,
        data: valueMoM.map(v => +v.toFixed(2)),
        smooth: 0.2,
        symbol: 'none',
        lineStyle: { width: 1.5, type: 'dotted', color: COLORS.red },
