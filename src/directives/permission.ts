 ...
 *
 * 基于 userStore.buttonPerms（后端 /auth/buttons 返回的完整路径权限标识符集合）
 * 实现声明式的按钮/DOM 元素显隐控制。
 *
 * 使用方式：
 *   <!-- 单个权限标识：用户必须拥有 'sys:user:add' 才能看到此按钮 -->
 ...
 *
 * 与 useButtonPermission hook 的区别：
 *   - v-permission 指令：模板声明式，自动控制 DOM 显隐
 *   - useButtonPermission hook：脚本式，用于 setup 中的逻辑分支判断
 */
 ...
 *   - 字符串 → 精确匹配 userStore.buttonPerms 中是否包含该值
 *   - 数组 → 只要数组中任意一个值存在于 buttonPerms 中即通过（OR 逻辑）
 */
function checkPermission(perm?: string | string[]): boolean {
  // 从 Pinia 中获取当前用户的按钮权限标识集合
